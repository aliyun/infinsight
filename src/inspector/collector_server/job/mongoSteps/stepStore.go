...
func chooseRing(prevStart, curStart uint32, ringCount uint32, ins string) (uint32, uint32) {
	if prevStart+ringCount-1 > curStart {
		glog.Warningf("step[step-store] instance-name[%s] with service[mongo] interval [%d, %d] illegal",
			ins, prevStart, curStart)
		return prevStart, curStart
	}

	if (curStart-prevStart)%ringCount != 0 {
		glog.Errorf("step[step-store] instance-name[%s] with service[mongo] current-ring-start[%d] != previous-ring-start[%d]",
	...
	if prevStart != curStart-ringCount { // gap is 1 ring
		glog.Errorf("step[step-store] instance-name[%s] with service[mongo] the gap of current-ring-start[%d] and previous-ring-start[%d] bigger than 1 ring count",
			ins, curStart, prevStart)
		prevStart = curStart - ringCount
	}
