...
type StatefulCmdable interface {
	Cmdable
	Auth(password string, authCommand ...string) *StatusCmd
...
func (c *statefulCmdable) Auth(password string, authCommand ...string) *StatusCmd {
	auth := "auth"
	if len(authCommand) == 1 && authCommand[0] != "" {
		auth = authCommand[0]
	}
